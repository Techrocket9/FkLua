...
- **2.1.x API surface** is one line away: `api = "2.1.14"` in `fklua.toml` (or `--api=2.1.14`), then `fklua gen-bindings && fklua lock`. Every supported description is committed, so changing the pin needs neither the game nor the network; at 2.1.14 the bindings cover 4,840 of 4,842 members with 224 events. On Steam, 2.1.x is the `2.1.14` entry under the game's Betas tab; the in-game test scripts pick the engine up through `FACTORIO_BIN` or the default Steam path.
