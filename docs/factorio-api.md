...
The whole runtime API is bound in both languages, member id for member id. Against the default **2.0.77** API pin: 4,255 of 4,262 members, 219 event payload structs (every event the description declares), 1,329 inherited forwarders (so `LuaEntity` has `LuaControl`'s `position` and `get_inventory`), 1,137 `defines` accessors, 11 class operators plus the write half of the two that have one, the three global functions, and 240 `<Name>Into(dst, ...)` variants that let an array return land in a buffer you already own. Seven members are deferred: five that take a Lua function as an argument, which a compiled guest has no way to supply, and two whose name collides with another member of the same class. The counts are committed data in `api/<version>/census.json`, regenerated with the bindings and gated by `gen-bindings --check`; read them from there rather than from this page.
