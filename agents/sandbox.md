...
| **A jump spans at most 131,071 VM instructions** | `MAXARG_sBx`. Past it the parser refuses the whole chunk with `control structure too long`, pointing into generated Lua and naming nothing in the guest. **Verified**: 131,071 loads, 131,072 is rejected. And a CHAIN of short jumps counts as one long one — `::A::` immediately followed by a bare `goto B` merges the two pending lists — so the bound is on the jump a chain RESOLVES to. See "A jump is 18 bits" below. |
...
Five things about it that are easy to get wrong, and each has already cost something:
...
- **A CHAIN OF SHORT JUMPS IS ONE LONG JUMP, so "every hop is under the limit" says nothing.** `::A::` immediately followed by a BARE `goto B` is not two jumps: `luaK_patchtohere` puts the jumps pending at A into `fs->jpc`, the next instruction emitted is the relaying jump, and `luaK_jump` concatenates the two lists — so `A → B → C → L` is patched as ONE jump from every entry point straight to `L`. Measured, every hop 50,000 instructions: a bare 2-hop ladder loads, a bare **4**-hop ladder is refused, and a **10**-hop ladder spanning 500,000 loads once **one ordinary statement** sits between each label and its goto, because any instruction at all discharges the pending list through `dischargejpc`. Only a BARE goto chains — `if <cond> then goto X end` emits the test first — and blank lines, comments and further labels emit nothing, so they do not separate either. This is what the emitter's relay is built on and what its check had to learn to measure; see [`agents/codegen.md`](codegen.md).
- **The diagnostic names the GENERATED file and nothing in the guest.** Measured in a real Factorio 2.1.16: `Failed to load mod "fk-jumplimit": __fk-jumplimit__/fk_data.lua:84: __fk-jumplimit__/fk_data_module.lua:163449: control structure too long near '('`. It carries a generated file and line, and the token the parser was holding when the pending gotos were patched — whatever follows the label, so for a guest often `near 'trap_unreachable'`. There is no route from `fk_data_module.lua:163449` back to the Go or Rust that produced it. That is the whole reason [`agents/codegen.md`](codegen.md) has a package-time check for it.
