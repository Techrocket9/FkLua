...
**And it must not carry a COLLECTOR, which is the third property `CheckDataModule` enforces.** `factorio.CollectorSurface()` -- `fk_gc_step`, `fk_gc_dirty_base`, `fk_gc_dirty_cap` -- is a PRECONDITION for a control guest and a REFUSAL here, because the same three exports mean the opposite thing at a stage that has no tick to pace a collection from and no state to survive. **Any ONE of the three is enough**, since control.lua binds all three or none. It is easy to reach by accident in both languages and needs no typo in Rust: cargo's v2 resolver unifies `--features fk/fkgc` across every package built in one invocation, so a workspace-wide build of a collected control guest carries the data crate with it; the Go form is `-gc=custom` plus an fkgc import left in a data guest copied from a control one. Filed by WormholeBelts (FKLUA-GAPS item 5). Measured on this repo's own example, the same source both ways: **599,178 bytes of generated Lua against 676,282, +12.9%** (a delta of 77,104), both packaged with exit 0 and no line saying so before this check existed. The measurement is ENGINE-FREE and re-runnable: `cargo build --release --target wasm32-unknown-unknown -p datastage` with and without `--features fk/fkgc`, then `fklua compile --persist=none --gc=leaking` on each build, which is the flag set `fklua mod` emits a data module with. Read the size off `compile` rather than off `mod`, because the collected arm is now refused by this very check; on the clean arm the two agree to the byte. **The wasm sizes are deliberately not quoted anywhere** -- `debug = "line-tables-only"` embeds the absolute build path, so they move with the length of the checkout's path and are not a fact about the build. The share is guest-dependent (the filing mod measured +3.1% on its own), so the message says "several percent" and cites the example for the figure. It is also not purely dead code: the Rust feature swaps `fk`'s single `#[global_allocator]` and Go's `-gc=custom` routes allocation through fkgc, so the stage runs under a heavier allocator it has no use for, behind a collector nothing can pace. The message is the two-sided shape the `--gc` refusals use and names both remedies by language. The `--report` kind stays `data_module`.
...
**env(5) is `defines.prototypes` read live at the stage, not baked from a description.** It is present at both stages in a real Factorio (measured, 2.1.17: 65 base types, `defines.prototypes.item` holding 21 derived), it is what "every kind of item prototype" needs and `data.raw` alone cannot answer, and reading it live is what keeps this out of the pin axis entirely -- the alternative was baking it from `prototype-api.json`, a description `api/<version>/` does not even carry. The engine's shape is `base -> { derived -> 0 }` with the zeros carrying nothing, so the arm crosses `base -> SORTED ARRAY of names`: an array because the dummy values would be noise, sorted by the shim because `write_sorted` orders dictionaries and deliberately not arrays. Absent `defines` reads as an empty map rather than raising, env(3)'s own settings-stage answer. Guest surface: `DerivedTypes(base)`/`derived_types` and the inversion `BaseType(derived)`/`base_type`, both off one cached read. *Enforced by `TestDefinesPrototypesCrossSortedAsArrays` (both levels' order pinned) and `TestAbsentDefinesReadsAsAnEmptyMap`, plus the `transport-belt is an entity` log line the end-to-end and mirror tests require.*
...
**And the convention has a GUEST half since 2026-08-31: `fkdata.raise(ptr, len)`.** FkRecipes' dogfood report was the ask: a validating guest -- a cycle detector, a presence ladder -- had no way to put its own diagnostic where the player looks, because a guest panic surfaces as `fklua trap: unreachable` with the real message surviving only as a log line above it. `raise` routes the guest's message through the same `fail()` every host-detected failure uses, so it arrives verbatim with the stage prefixed, and the call never returns. Raw `(ptr, len)` rather than tier 2, deliberately: it is `fk_log`'s shape, allocates nothing and decodes nothing, and a raise path that could itself fail on an allocation would be a diagnostic that dies of the disease it reports. `Raise(msg)` in Go (documented as not returning, with a hard stop behind it), `raise(msg) -> !` in Rust. *Pinned by the "the guest's own raise" case of `TestAFailureRaisesNamingTheStageAndThePath`, red-proven against the seven-import shim.* **And the adoption found the half the doc comment did not say (2026-09-01).** "The stage name is prefixed" named the fact and not the text, so FkRecipes, whose messages already carried a stage, read `fklua: at the data stage, fkrecipes: at the data stage, two technologies share the name hardened-tips` in the player's game with nothing failing. The contract is now spelled where a library author reads it: the Go and Rust doc comments, the shim header, `docs/data-stage.md` and the `init --library --data` scaffold all state that the message lands verbatim behind `fklua: at the <stage> stage, ` and therefore carries no stage of its own, and each shows the resulting line. Two guards hold it: the raise case of `TestAFailureRaisesNamingTheStageAndThePath` pins the EXACT line rather than two substrings (red-proven by changing `fail()`'s separator), and `TestEveryStatementOfTheRaisePrefixIsTheShimsOwn` in `cmd/fklua` reads the prefix out of `fail()` and requires every statement of it in code comments, docs and scaffold to match, so the five quotations cannot drift from the one that runs.
...
- ~~**`fklua init` scaffolding for a data guest.**~~ **DONE.** The condition was "once the shape has survived one real port", and it survived two, one per language: BetterBeltBalancer's `guest/go/data` (a second main package in the control guest's module, exactly D1's shape) and WormholeBelts' Rust data crate (its own cargo invocation, no feature, fkdata not fkapi). `fklua init NAME --data` writes it for EVERY declared language, plus the `data_module` key -- one key however many languages, taking the first in `lang` order with the other artifact named in a comment beside it, because a mod has one data stage. The Rust arm adds a third workspace member and `fkdata` + `fklog` to `[workspace.dependencies]`, and every printed cargo line names its package with `-p`: an unqualified `cargo build --features fk/fkgc` over a workspace that now contains a data crate is precisely the unification that CheckDataModule refuses. That is TWO cargo lines in a Rust-only project, and one beside a tinygo line in a two-language project whose key names Go, which is why the scaffolded Rust header states it as `-p` rather than as a line count. **AND THE PRINTED BLOCK IS NOW THE SCAFFOLDED ONE, character for character**: both render `initRecipeCommands`, so a `--data` project prints one three-line block per language and each block builds the data module `data_module` NAMES rather than the one that language produces. The two-language rule reached the scaffolded headers a round before the printed steps, and until it did, pasting the non-key block ended on `fklua mod: data module: open <path>: no such file or directory` -- an open error naming a wasm nothing in the block had built. The same data build therefore appears in BOTH blocks on purpose: each block is a complete recipe, and building it twice is a cached no-op where not building it is that error. Held by a structural check over both `lang` orders (hand-spelled command lines, no toolchain) and by an end-to-end one that RUNS each printed block with every wasm cleared first, so a block cannot pass on the other block's output. Without the flag the scaffold is byte-identical, gated by comparing a `--data` run against a plain one file by file. Filed by WormholeBelts (FKLUA-GAPS item 2).
...
- **`settings-updates` and `settings-final-fixes` are deliberately unwired.** Factorio's settings family is three rounds and `StageHooks` carries only the first: nothing has needed to patch another mod's SETTING the way `data-updates` patches another mod's prototypes, and wiring one is a one-row `StageHooks` addition plus a stage id the day something does. Written down 2026-08-30 so the omission is a decision rather than a gap nobody noticed; note the stage ids are the ABI, so a new round APPENDS an id rather than renumbering.
