...
| **P9** | **`localised_print`** is BOUND since `M.GFUNC` (`agents/abi.md`, "The three global functions"), and it did not land for this library's sake: UDP-only had already deleted the only reason phase 1 had to reach for it, and what re-opened the kind was `log` and a profiler. Nothing here uses it. | closed elsewhere |
