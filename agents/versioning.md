...
- **Named types, from member signatures AND from event payloads.** A guest calling a member whose argument is a `MapGenSettings` breaks when `MapGenSettings` gains a field, even though the member itself did not change — so the check collects every named type reachable from the signatures it uses, recursively through structs, arrays and dictionaries. Without that it would miss the *dominant* shape of the 2.0.77 → 2.1.12 breakage, which is table concepts gaining fields. The **payload** half is the same argument one door along, and the event loop did not make it: an event's scratch block is laid out from its fields, so a concept one of them names gaining a field moves every offset after it in a reader the guest was compiled against, while the diff says nothing at all against the event's own name.
  ...
  **No committed pair can produce the finding**, which is why it is fixtured rather than driven from real data: **zero** concepts named by any event payload moved between 2.0.77 and 2.1.12, 2.1.14 or 2.1.16. So the impacted case is a synthetic `Report`/`EventReport` in `internal/factorio/apicheck_test.go` — a subscribed event whose payload names a concept the diff calls broken, asserted to arrive as `match: "concept"` and *not* `event`, since the event did not move, with a second unsubscribed event's concept required to stay OUT of the surface, which is the control an implementation collecting every payload would fail. The half that does run on real data is `TestSomeConceptsAreReachableOnlyThroughAnEventPayload`, which derives an event-only concept from the committed description and requires a one-event surface to carry it: that is what anchors the fixture to something that exists. **Confirmed red** by deleting the payload loop — four assertions, the real-data one among them (`a guest subscribing to CustomInputEvent depends on "SelectedPrototypeData" and the surface does not carry it: []`).
