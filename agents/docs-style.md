...
FkLua is released under the MIT License (`LICENSE` at the repository root). Say so plainly, link the file, and do not imply anything else anywhere. Two committed inputs are third-party work under their own terms and the README names them: `testdata/spec/` (the WebAssembly specification test suite) and `third_party/lua-5.2.1/` (Lua, fetched at build time). A document must not claim MIT for those.
