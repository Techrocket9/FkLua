...
- **Round 3 SHIPPED on 2026-08-30**, and its record is CLAUDE.md's "The seam
  ...
  - the **simulation bridge**, verified to exactly the extent it can be. The
    ...
    **Nothing headless runs a simulation** -- no flag, no mention in any log --
    so the last hop was closed in the field instead, 2026-08-30 on 2.1.17: a
    tips entry built to the recipe played, and the guest staged the scene with
    `in-simulation=true` against a console control's `false`. And **the engine does not validate the init string at load**,
