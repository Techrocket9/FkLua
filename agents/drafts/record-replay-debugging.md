...
|---|---|---|
| **A** | `fk_module.map.json` beside `fk_module.lua`: each emitted guest function's Lua line range, wasm function index, name-section name (Rust demangled best effort), optional `src`/`line` from DWARF. Version field `"fklua_map": 1`. The toolkit's DAP proxy annotates Factorio debug-adapter stack frames with guest function names. | **SHIPPED.** Emitted by default by `fklua mod`, `--no-map` opts out, and the data module gets its own `fk_data_module.map.json`. Function-level attribution only: no breakpoints, no stepping, no variables. Two deviations from the draft contract: the synthetic name is FkLua's existing `func[N]` spelling rather than `f142`, and a demangled entry carries the raw symbol in an extra `mangled` field. |
...
**What it costs.** Worse DX, on two axes. Wasm DWARF support is thinner than native — stepping generally works, variable inspection is patchy and optimisation-dependent, and `-opt=2` is mandatory for this project's guests rather than optional. And it needs the guest to **carry** DWARF, which Tier A has now measured for both toolchains: TinyGo ships full DWARF at the flags this project's guests are built with, and the Rust scaffold's release profile gained `debug = "line-tables-only"` (`debug = true` was not needed and costs 21.6× the wasm against line tables' 3.37×). What Tier B needs beyond that is line-table granularity SURVIVING `-opt=2`, which is a different question and still open.
...
- **DWARF at line granularity, actually present in the shipped wasm.** Tier A's `src`/`line` needs only `DW_TAG_subprogram`'s `decl_file`/`decl_line` and is explicitly best-effort; the line tables themselves are present in both toolchains as Tier A ships them, and what is unverified is whether they still correspond to anything after `-opt=2`.
