...
The count includes **282 `MemberGetEq` entries**, the host-side string predicate described below: a third member KIND, one per plain-string attribute. And **11 class-operator entries** under three more kinds — `M.IDX`, `M.LEN`, `M.SELF` — which are Lua's `obj[k]`, `#obj` and `obj(...)`. See "Class operators" below. And **3 `M.GFUNC` entries**, which are on no class at all: `log`, `localised_print` and `table_size`. See "The three global functions".
...
### A retained handle has an OWNER now, and the ownership surface is asymmetric on purpose

**Filed by WormholeBelts as item 6**, and the report is what makes it a gap rather than a preference: the first Rust guest to hold handles across events leaked on **three paths a test would not reach** — a build that had retained three handles before a fourth create failed, an early return past the release, and a helper that returned a null handle its caller went on holding. Underneath all three sat the same absence: `retain()` and `release()` were a pair the author had to balance on every path, and there was **no way to ask which space a handle was in**, so a double retain was silent and the only thing an author could look at was the raw number.

**The predicates are guest-side, free, and in both languages**, because the split is by number: `Global()`/`Persistent()`/`Transient()` in Go, `is_global`/`is_persistent`/`is_transient` in Rust, one compare each and no host call. The two split points are `Object::FIRST_DYNAMIC` and `Object::TRANSIENT_BASE` in the Rust preamble and `FirstDynamicHandle`/`TransientHandleBase` in the Go one, **one declaration per language**, pinned against `fk_abi.lua`'s own `FIRST_DYNAMIC` and `TRANSIENT` by `TestHandleSpaceConstantsMatchTheLuaABI` — which reads the numbers out of the ABI source, in `TestKindNumbersMatchTheLuaABI`'s shape, and holds **a property over each constant's DEFINITION and its USES**: exactly one declaration per constant per language, and then each of the six predicate BODIES must NAME the constants it compares against and must spell neither split point as a literal, with one-match anti-vacuity per predicate. That covers `FIRST_DYNAMIC` as well as `TRANSIENT_BASE`, which is the half a count of literals over the whole preamble could not (10 is far too common a number to count), and prose cannot trip it: a doc comment that merely mentions `0x40000000` is green.

**THE PREDICATES ANSWER THE SPACE QUESTION AND THE GUARD ANSWERS THE OWNERSHIP ONE, and conflating the two was this round's own defect.** The first cut documented `is_persistent` as *this guest retained it, it lives in the save, and this guest owes it a release* — which is an ownership claim on a pure range test, and the commit's own golden transcript refuted it: `classify … 10=persistent 1073741823=persistent` is the predicate saying that about two numbers the example HAND-BUILT and nothing ever retained. It cannot distinguish a live slot from a released one, from a fabricated number, or from a slot an earlier build discarded with the heap; the only way to learn a handle is dead is to call something and read `ERR_BAD_HANDLE`. So the wording everywhere is now what the code does: **the number NAMES A SLOT in the persistent space**. The gap report's "no way to ask whether a handle was already retained" is answered in two halves rather than one — the space predicates for the SPACE, and `Retained` for the OWNERSHIP, because a guard is the only thing that was present when the slot was minted.

**THE NINE GLOBALS ANSWER FALSE TO `is_persistent`, AND THAT IS A DECISION** — one that survives the correction above, because it never needed the ownership reading. It stands on the ranges alone: **a global is in neither dynamic space**. 1..9 sit below `FIRST_DYNAMIC`, are bound at load and are never allocated or freed, so the three predicates partition every handle exactly the way the table at the top of this section does, and the null handle answers false to all three. Folding the globals in because they also outlive the dispatch would put them in a range they are not in — and, as a secondary matter, would have `is_persistent` say true of `game`, whose `release` is `ERR_BAD_HANDLE`. The outlives-the-dispatch question is `!is_transient()` on a live handle, and the ABI's own vocabulary never called 1..9 persistent either.

**NONE OF THE THREE SAYS A HANDLE IS LIVE, and the first cut said otherwise twice — once per predicate that had a spare sentence.** Both were measured against the verbatim `fk_abi.lua` through `bin/lua52f` rather than argued. (1) `is_transient`'s doc said a number *left over from an earlier dispatch names nothing and answers `BAD_HANDLE` on the next call*, and it is the opposite that is true and in the dangerous direction: `M.clear_transient` RESETS THE COUNTER (`transient = {}; nextT = TRANSIENT`), so `0x40000000` is handed out again on the very next dispatch and a kept number resolves with `OK` to a DIFFERENT object — `dispatch 2: B is 1073741824, same number: true` / `A's old handle reads B, status 0` / `retaining A's old handle: slot 10, holding B, status 0`. The retain SUCCEEDS, so `retained()` cannot refuse this and no predicate could: all three are compares on the number. `BAD_HANDLE` is the answer only where the current dispatch has not allocated that far. The rule is therefore the caller's and is stated as one in both preambles, on `retained()` itself, in `docs/factorio-api.md` and in `docs/from-lua.md`: retain INSIDE the dispatch that produced the handle. `fk_mod.lua` has said what it costs since the nesting fix — *clear_transient restarts the id counter, so the outer handle could come back pointing at a different object, which in a lockstep game is a desync rather than an error* — and `internal/factorio.TestAStaleTransientHandleNamesSomebodyElsesObject` is now the tripwire under the sentence, pinning both halves (the reuse, and the not-that-far case that answers `BAD_HANDLE`). Red-proven twice against the handle table itself, each injected and reverted: dropping `nextT = TRANSIENT` from `clear_transient` reddens the reuse half (`B is 1073741825, same number: false`, `A's old handle reads nil, status 1` — which is exactly the sentence that was removed), and dropping `transient = {}` instead reddens the other half (a number the new dispatch never reached still reads its old object). (2) `is_global` claimed the globals range was *the one range where membership also says the handle is live*. It is not: `M.get` and `M.retain` resolve a global through `genv[name]` on every access, `genv` is nil until `bind_globals` runs, and `game` does not exist while control.lua is loading — which is where a guest's package initialisers run, as `arm_deferred`'s own comment says — so `M.get(2)` answers `nil, 1` and `M.retain(2)` answers `0, 1` in both of those windows and only a live env gives `2, 0`. What is true of 1..9 is about the NUMBER: never allocated, never freed, so never reused for something else, which is the one thing neither dynamic range gives. Both preambles now say that and nothing more, `docs/factorio-api.md` carries the same paragraph, and `internal/factorio.TestAGlobalIsNotLiveUntilTheGameBindsIt` pins the three windows in one transcript — red-proven twice, once per half: dropping the `genv[name]` guard from `M.retain`'s global path makes an unbound global retain answer `2, 0`, and dropping it from `M.get`'s makes an unbound global resolve.

**`Retained` is Rust-only, and that is the accepted asymmetry class** — the `agents/guests.md` "where Rust says it better" table, not a Go gap. `Object::retained(self) -> Option<Retained>` retains and takes ownership; `Deref<Target = Object>` makes every generated method reachable through it (`LuaEntity(*guard)`); `Drop` releases; `into_object(self)` hands the handle back **without** releasing, by `core::mem::forget`. Go gets `defer o.Release()` and nothing else: the language has no destructor, and a wrapper type would hide the same defer without removing the obligation.

**A GUARD IS BORN AT THE PROMOTION AND NOWHERE ELSE, which is what makes two owners of one slot UNREPRESENTABLE rather than merely discouraged.** The first cut returned a bare `Retained` and withheld `Copy`/`Clone`, with a comment claiming that closed the shape. It did not, and `Deref` is what re-opened it: `Object` derives `Copy`, so `*guard` is a full owned `Object` and `Retained::new(*guard)` compiled in one line, producing a SECOND guard over ONE slot — `M.retain` is idempotent for an already-persistent handle and allocates nothing. **The consequence is ABA rather than a benign double release, and the LIFO free list makes it the default case**: guard A drops, the slot goes to an unrelated guard C on the very next retain, B's drop then frees C's slot, and a fourth owner lands on it while C still names it. Measured end to end through the verbatim `fk_mod.lua`/`fk_abi.lua`, and kept as `internal/factorio.TestReleasingOneSlotTwiceFreesAnotherOwnersObject`: `B releases, status 0` (not `ERR_BAD_HANDLE`) and `C reads its slot and gets D`.

So `retained()` takes **only a TRANSIENT handle**, the one case where the host mints a FRESH slot for every call, and answers `None` for each of the four shapes it cannot own: an already-persistent handle (the number names a slot in the persistent space, and retain mints nothing there, so a guard over it could not be the slot's only owner; whoever does own it, if anyone, owns it by hand or through another guard, and the release that came with the guard would free whatever the next retain took — the honest verb there is *adopt*, and the binding offers none), a global (owned by nobody; `Drop` would be a discarded `ERR_BAD_HANDLE`), the null handle, and a **retain that failed** and came back 0, which is the report's third leak — "a helper that returned a null handle its caller went on holding" — rebuilt inside the new API. `Retained::new` is the same function spelled as a constructor and carries the same refusals. Not `Copy` and not `Clone`; and **a copy taken through `Deref` is a BORROW** — fine to read and to pass to a call, not to outlive the guard, release by hand or store past its scope, and unable to become a second guard because `retained()` refuses a persistent handle.

Three further things are worth stating rather than discovering. A guard in a static map **survives a save** with the handle it names, because the guest heap is what Factorio serializes and the persistent table is aliased into `storage` beside it — **now exercised rather than argued**, by a second leg of the ownership test that runs two sessions in one interpreter with `storage` deepcopied between them, at `--persist=table`, and has the guest read the parked guard back through `fk_on_tick` and CALL something on it (`--persist=none` reddens it to `slot=0 nauvis=gone`). On a **rebuilt** guest the heap is discarded and the persistent table is discarded under the same gate, so the guards die with the numbers they were holding. And `Drop` is **deterministic because it is guest-driven**, running where the scope ends **on every peer that runs that scope** — which is not a property a finalizer would have, and is not a property `fk_after_load` has either: **no retain, release, construction or drop may happen there**, because `script.on_load` runs on the joining client alone while `storage.fk_handles` and (under `--persist=table`) the guard itself are both inside what Factorio checksums. That is a desync, not a leak, and it is stated on the type, in `docs/factorio-api.md` and in `docs/from-lua.md`. `panic = "abort"` means no `Drop` ever runs during a panic.

**RELEASING ONE SLOT TWICE IS NOT HARMLESS, and the first cut said in three places that it was** (both preambles' `release` doc and `docs/factorio-api.md`). `M.release` checks that the slot is OCCUPIED, not by whom, and the free list is LIFO, so a stale release lands on a live slot and answers `OK`. The raw `retain`/`release` pair is the escape hatch and its doc now says so: exactly one release per slot, and hold a `Retained` if you would rather not have to know.

**THE TABLE-SIZE ACCESSOR IS DEFERRED, DELIBERATELY, AND THE PROXY IS THE SLOT INDEX.** The gap also asked for a way to read how many entries the handle table holds, so a leak test could assert on it. That is a **new host import** — an ABI change touching `fk_mod.lua`, both `fk` libraries and the report surface — bought for a test's convenience, and it is not needed, because the slot index `retain()` hands back is already a deterministic proxy. The persistent free list is **LIFO during play**, so a released slot is the very next one handed out; a release that did not happen shows up as the next retain taking a NEW number; and a high-water mark over a churn bounds the live count. `TestHandleOwnershipReadsTheSameInBothLanguages` is written on exactly that: the guard's slot is 11, the next retain gets 11 back, an `into_object`ed slot is 12 and the retain after it gets 13. Removing the release from `Drop` moves all three, which is the red proof. Note the free list is LIFO in play and **rebuilt ascending on load** (see above), so the proxy is a within-session one; nothing here reasons across a save.

**Exercised in both languages against one transcript, plus a Rust-only tail.** `guest/go/examples/handles` and `guest/rust/examples/handles` are line-for-line twins down to a marker in `fk_on_init`, and `internal/guest.TestHandleOwnershipReadsTheSameInBothLanguages` drives both through the verbatim `fk_mod.lua` and `fk_abi.lua` against one stub and requires identical output over the shared part — which is the only mechanism available, since the guard exists on one side and the defer on the other and what has to agree is the OBSERVABLE. **What follows the marker is asserted on the Rust arm alone, and the split is there so the test stops implying otherwise**: `retained()` returning an `Option` is an API Go has no half of, and the same was already true of `into_object`, which the Go example can only approximate with a plain retain whose release it keeps. The tail pins the four refusals in one line and then the ABA itself: A owns slot 14, both spellings of a second guard over A's own slot answer `None`, A drops, C takes 14 back and **proves it still owns it by CALLING something** rather than by holding a number.

Red proofs, each injected and reverted, control green before and after. The original four still reproduce: `Drop` without its release (three lines move), the transient split constant off by one in both preambles, `into_object` without its `mem::forget`, and `is_persistent` widened to include the globals. Six more came with the review fixes. **`retained()` with both gates removed** — the pre-fix shape wrapped in `Some` — reddens five lines, and the numbers say what the defect costs: `refuses owned=false global=false null=false failed=false`, `second guard over A new=true retained=true`, and **A's slot becomes 12 rather than 14**, because the temporary guards minted on the refusals line released the `into_object` slot the guest was managing by hand. **Each gate alone** reddens its own column (`failed=false` with the null-retain gate gone; `owned=false global=false` with the transient gate gone). **The exactly-once literal tooth is replaced** by a per-predicate one, red-proven both ways: inlining `10` for `FIRST_DYNAMIC` in both preambles is now RED in both arms where the old count could not see it at all, inlining the transient literal is red on both teeth, and a doc comment that merely mentions `0x4000_0000` is GREEN where the old form went red. **The ABA test's expectation flipped to the old doc's claim** (`B releases, status 1`, `C reads its slot and gets C`) is red, which is the measurement that the false sentence was false. **The save leg at `--persist=none`** reads `slot=0 nauvis=gone`.
...
### ...and WHAT the engine said — `fk.last_error`

**A status is an i32 and a message is not, so the two cannot come back together.** `fk_abi.lua` has recorded the engine's own text in `M.last_error` on every `ERR_CALL_FAILED` path since the ABI existed, and until 2026-08-24 **no import carried it**: the only consumers were this repo's own Go tests, so a guest could see that the API refused and never why. `Status.Error()` says *"the Factorio API raised"* and stops there.

```
    fk.last_error(ptr, cap) -> len
```

Copies at most `cap` bytes into guest memory at `ptr` and returns the **FULL** length, so a caller with a fixed buffer learns from the return whether it saw the whole message and can ask again with room. `0` means the last call did not fail.

**`M.call` CLEARS THE SLOT ON THE WAY IN**, which is what makes the contract sayable in a line: *after a call returns `ERR_CALL_FAILED` this is that call's message, and after any other outcome it is empty*. Without it the value would mean "whatever failed last, ever" — and a guest reading it after a call that SUCCEEDED would get a stale sentence about some other member, which reads exactly like a fresh one. One upvalue store per host call, at the single entry point. The one seam is re-entrancy: `invoke` can raise an event synchronously, so an inner failure under an outer success leaves the inner message standing, which is the honest answer.

**What it is FOR is a tripwire, and that is why it is worth an import.** Factorio refuses a documented subset of `script.raise_event` outright (see the headless walls below), and a suite asserting only `ok == false` cannot tell *refused because that event is not raiseable* from *refused for some other reason* — so the day the engine starts allowing one, the run goes on passing over a path that has silently become testable for real. BetterBeltBalancer asserts the exact text, `on_player_mined_entity (ID 76) (76) can't be raised through script.`, for precisely that.

**It is diagnostic and a guest must not branch on it**, which the two wrappers' doc comments say in as many words: the wording is an engine implementation detail a point release may reword, and a mod that behaved differently because of one would behave differently on two Factorios. A TEST asserting the text is the honest exception.

The two wrappers **differ, and neither is a transliteration of the other**. Go's `LastError() string` reads into a package-level `[256]byte`, because `var b [256]byte` whose address is taken does NOT stay on the guest's stack under TinyGo — the `ptrtoint` that makes an address crossable defeats LLVM's promotion, so a local would be a permanent heap allocation per call. Rust's `last_error() -> Vec<u8>` uses a stack array, because rustc has no such problem, and it returns BYTES rather than a `String`: a Lua string is an arbitrary byte sequence and `from_utf8_lossy` rewrites every byte outside UTF-8 while changing the length — the same lesson the 738 generated readers taught, stated in the one place a guest touches raw engine text. Both grow to a second call for a message over 256 bytes, which is what the full-length return is for.

*Enforced by `TestTheEngineSMessageReachesTheGuest` (`internal/guest`), which builds `examples/lasterror` in BOTH languages and drives each through the verbatim `fk_mod.lua` and `fk_abi.lua` against one stub whose `game.__index` raises — four legs, one set of expectations: the exact refusal text with its status beside it, the EMPTY case before anything failed, a SUCCESSFUL call clearing the slot, and a 300-byte message coming back whole through a 256-byte buffer. The status crosses as a NUMBER because Go's error convention prefixes `fklua: ` and Rust's `Status::as_str` does not. Confirmed to fail twice: with `M.call`'s clear removed the after-OK leg reads back the refusal, and with the import returning what it COPIED rather than the length the long leg reads `len=256 head=HEADHEAD tail=xxxxxxxx` — the right head and the wrong tail, which is what a silently truncated message looks like.*

**And verified in a real Factorio, both languages, byte-identical** (`GUEST=api ./scripts/run-guest.sh`, `LANG_=rust`): the index-assign leg's refusal now logs `last-error: [LuaCustomTable doesn't contain key fklua-no-such-setting.]`, which is the engine's own sentence and not a stub's.
...
### The three global functions — `M.GFUNC = 9` — **built, both toolchains**

**`log`, `localised_print` and `table_size` are on no class, and for as long as there have been binding generators they were bound by nothing.** `global_functions_bound: 0` sat in every `census.json` under a comment saying that was a decision rather than an omission — which is the only reason it could be re-taken rather than rediscovered. It reads **3** now, at every pin this checkout owns.

**What re-opened it is `log`, and the reason is narrower than "a global function is missing": it is THE ONLY WAY TO READ A LuaProfiler'S DURATION.** `LuaProfiler`'s complete member set is `add`, `divide`, `reset`, `restart`, `stop`, `object_name`, `object_name_is` and `valid`. Not one of them returns the number. The engine renders it only when the profiler is an ELEMENT of a `LocalisedString`:

```
    local p = helpers.create_profiler()
    ...work...
    p.stop()
    log{"", "[TAG] timing ", p}         -- ... Duration: 12.368959ms
```

BetterBeltBalancer publishes every timing figure in its documentation by regexing exactly that out of `factorio-current.log`, and porting its harness to a guest left it with no way to produce the line at all. `fk.Log` takes a plain string and a plain string cannot carry an object.

**A KIND RATHER THAN AN IMPORT, and the cheaper ask is on the record because it was refused.** The alternative filed beside this one was `fk_log_dyn(ptr)` — one import, host side `log(read_dyn(ptr))`. It is genuinely cheaper and it serves exactly one of the three: `localised_print` would still be unreachable and `table_size` would need a second import. As a KIND it inherits `decode_args`, the trailing-argument trim, `encode_rets`, the `ERR_NO_MEMBER` degradation and the member-id scan that prunes the shipped table, with no line of its own in any of them — the same argument `MemberGetEq` made, and `M.call` needed **no change at all**.

**Its branch runs BEFORE the handle is resolved, and that is the whole of what is new in `M.invoke`.** Every other kind starts with `M.get(h)`; this one has no receiver, so the generated binding passes **0** and nothing reads it. What the marshalling already carried was everything else: a `LocalisedString` with an Object in it is `Value` with `TagObject`, `writeDyn` emits `DYN_OBJ`, and `read_dyn` decodes it through `M.get(handle)` — the codec's own comment has named the LocalisedString case since tier 2 existed.

| | Go | Rust |
|---|---|---|
| `log(LocalisedString)` | `fkapi.Log(Value) error` | `fkapi::log(&Value) -> Result<(), Status>` |
| `localised_print(LocalisedString)` | `fkapi.LocalisedPrint(Value) error` | `fkapi::localised_print(&Value) -> Result<(), Status>` |
| `table_size(table) -> uint` | `fkapi.TableSize(Value) (uint32, error)` | `fkapi::table_size(&Value) -> Result<u32, Status>` |

Package-level in Go and free functions in Rust — there is no receiver to hang a method off. **The ids are APPENDED after every class**, which is a decision rather than the loop's leftovers: ids are dense indices into the member slice, so inserting three anywhere else renumbers everything below them and turns the golden diff into 8,000 lines of moved constants. At 2.0.77 they are 4260, 4261 and 4262 and nothing before them moved by one.

**Pruning is untouched by construction and is measured rather than argued.** The constant scan reads operand 1 of `fk.call` and a global function changes operand 0, so it knows nothing about the kind — and `examples/api`, which calls `Log` and `TableSize` and not `LocalisedPrint`, packages **20 members of 4,262** in 6,252 bytes, carrying `kind=9,name="log"` and `kind=9,name="table_size"` and not the third.

**Two small asymmetries between the backends, each for a reason rather than by oversight.** Go checks the emitted name against the generated TYPE names and defers on a collision; Rust does not, because Rust puts types and values in separate namespaces and such a check could never fire. And the Rust source is dedented one level on the way out, since every member body here is written for life inside an `impl` and these are not — cosmetic, and it keeps the two generated files readable side by side.

**`table_size`'s own case is still the weak one and it is bound anyway.** The argument that once justified it — sizing a dictionary without materialising it — is gone twice over: a dictionary RETURN crosses as a `(ptr, count)` pair slice, so the count arrives with the data, and a `LuaCustomTable` has a handle route whose `Length()` answers without the table crossing at all. What it counts now is a plain Lua table, which for a guest means a tier-2 value it built or one a callback handed it. It is bound because the kind is the cost and the third member is free; the description says outright that it does not work on a `LuaCustomTable`, and the generated doc comment says so too.

*Enforced by `TestTheGlobalFunctionsAreBound` (the members exist, on no class, with no `valid`, in both backends at one id each, appended last) and `TestAGlobalFunctionDispatches`, six legs through the real `fk_abi.lua` under `lua52f`: `log` with a nested LocalisedString carrying a `DYN_OBJ`, where the stub receives the ASSEMBLED Lua table and the profiler arrives as the identity the handle names rather than a copy or a number; **the same call with a dead handle, answering identically**, which is the only way to say out loud that the branch runs before `M.get`; `localised_print` resolving its OWN global rather than `log`'s; `table_size` counting and returning; a global this Factorio does not have coming back `ERR_NO_MEMBER`; and one that RAISES coming back `ERR_CALL_FAILED` with the engine's text in `last_error`. Confirmed to fail with the `M.GFUNC` branch removed — every leg answers **`ERR_BAD_HANDLE`**, because handle 0 is then resolved instead of ignored, which is the placement stated as a measurement.*

**And verified in a real Factorio, both languages** (`GUEST=api ./scripts/run-guest.sh`, and `LANG_=rust`): `examples/api` creates a profiler through `helpers.create_profiler`, times a loop, stops it and logs `{"", "global-fn: profiler ", p}` — and what comes back out of the game's own log is the engine's rendering, which is the entire point of the gap and the one thing no host-side stub can produce.

### The hand-binding escape hatch
...
### A `table | tuple` concept arrives in whichever form the engine sends

`canonicalUnion`'s **shape A** — one table option plus array shorthands — collapses to the table, and the header's justification for that was *"it is what a read returns, and a write accepts either"*. The first half was wrong, and it was wrong in the direction that produces a number rather than an error. **The description declares both forms because the ENGINE sends either**, and `Vector`'s own text says which one it picks: *"The game will always provide the array format."* `write_struct` looked its fields up by NAME only, so `{2.19, 0}` gave `nil` at `t.x` and `t.y` and it wrote a pair of **zeros with the presence byte set** — a plausible coordinate, status `OK`, nothing logged, nothing in `fk.last_error`.

**Measured on 2.0.77, and the two halves are DIFFERENT CONCEPTS, which is the evidence rather than a detail:** `LuaEntityPrototype::inserter_drop_position` read `0` tiles off the six inserter prototypes a base plus Space Age game defines, while the same reach measured off live entities in the same run was `2.19` tiles. The live reading goes through `LuaEntity::drop_position` and `pickup_position`, which are `MapPosition` at 2.0.77 and not `Vector` — a distance derived by subtracting two absolute positions, not a second read of the same member. So the reading that WORKED is the concept that arrives keyed and the reading that wrote zeros is the concept that never does: **the per-member choice, measured, in one pair of numbers.** Filed by WormholeBelts (item 8), which needed the prototype's number to compute an inserter's reach and could not tell the answer from a real zero. **Reproduced and closed in a real 2.0.77** through `examples/api`, in both languages: `prototypes.entity["inserter"].inserter_drop_position` logs `0.00,1.20` with the flag and `0.00,0.00` without it, byte-identical between the Go and Rust arms and across two benchmark runs. **And the NUMBER is asserted rather than read.** `run-guest.sh` fails only when NOTHING in its per-guest alternation matches, and its determinism leg only counts the lines that did appear, so a leg that vanished or logged a well-formed `0.00,0.00` passed the whole script — measured, with the fallback deleted: it printed the zeros, reported "all guest lines identical", and exited 0. The api guest now carries `inserter_drop_position` in the pattern (the member's own failure branch says that and not `shorthand struct`, so it would have disappeared from the output rather than reporting) and a `MUST_RE` the run has to match, red-proven the same way: the same deleted line now exits 1 naming the value it wanted.

**Ten concepts have the shape, at all five committed descriptions**: `BoundingBox`, `ChunkPosition`, `Color`, `ColorModifier`, `EquipmentPosition`, `GuiLocation`, `MapPosition`, `TilePosition`, `Vector`, `Vector3D`. `MapPosition` happens to arrive keyed today and `Vector` never does. The two are about 2.5x apart at 2.0.77 on either measure taken LIKE FOR LIKE -- 152 description references (`type` 127 + `read_type` 16 + `write_type` 9) against `Vector`'s 58 (49 + 7 + 2), or 16 attributes READABLE as it against 7 -- READABLE rather than declared, because `MapPosition` has one write-only attribute (`LuaEntity::crane_grappler_destination`) and `Vector` has none, so the declared counts are 17 against 7 and only one side of the contrast moves -- and not the 22x that comparing MapPosition's references with Vector's attributes suggests, which is an artefact of mixing two units. **And the per-member choice is MEASURED in this repository rather than transcribed from the filing**, which it was in six places until `examples/api` grew a second read: `LuaEntity::position` off the iron chest the example already builds at (8, 8), beside the Vector it already read. One real 2.0.77 run, both language arms, carries `shorthand struct: inserter_drop_position = 0.00,1.20` and `keyed struct: entity.position = 8.50,8.50` together -- and with `write_struct`'s positional fallback deleted the first reads `0.00,0.00` while the second is unchanged, which is one shape arriving in each of its two forms with only one of them needing the flag. Both lines are `MUST_RE`d, and `MUST_RE` is now ONE REGEX PER LINE with all of them required: an alternation would have been satisfied by whichever half survived. That is why a defect this old surfaced at the smaller member and not the larger one — **the population is not the blast radius, the engine's per-member choice is**, and nothing in the description says which members choose which.

**The generator marks the shape and the host honours it.** `FieldSpec.Positional` is set by `canonicalUnion` where shape A returns, carried through `LayoutStruct` onto `Placed`, and rendered by `placedList` as `,pos=true` on the `KindStruct` descriptor — **only when true**, so every other descriptor is byte for byte what it was. `write_value`'s `K_STRUCT` arm passes `f.pos` into `write_struct(fields, base, t, pos)`, which falls back from `t[f.name]` to `t[i]` **only when the name gave nil**. So a table carrying both spellings still reads by name, and nothing without the flag changes at all.

**The index is the tuple index by construction, and BoundingBox is what proves it rather than assuming it.** `LayoutStruct` places in the description's `order`, and the array shorthand is an ordering of the same members. BoundingBox's parameters arrive in the JSON as `left_top, orientation, right_bottom` and its `order` is `left_top, right_bottom, orientation` — which is the shorthand `{{-2,-3},{5,8}}` with the optional third member left off the end. An optional with no element opposite it reads nil from `t[i]` and stays **absent**, the same answer the keyed form gives.

**A nested positional struct carries its own flag**, which is why `pos` is a per-descriptor field and not a per-call argument: BoundingBox is positional and so is each of its two MapPositions, so `{{1,2},{3,4}}` has to resolve at both levels.

**Two things could misalign the index and BOTH are asserted against rather than assumed away.** The first is an OMITTED member: the placed index is the tuple index only while every declared member reaches the struct, and this generator does drop one class of field, a `nil`-typed member, which `typeMapper.omit` records and no committed description has inside any of these ten. Such a drop would shift every member after it against the array form while leaving the keyed form correct, so the two would disagree with nothing saying which was right. The second is the SHORTHAND ITSELF, which a placed count says nothing about: a concept declared `table{x, y} | tuple[float x3]` would place two fields, read the first two elements and drop the third with every other check green. So the generator test compares each concept's placed field count against the members its table option declares, AND walks every shorthand the description carries — no tuple longer than the table, element `i` the same type as the member `order` puts at index `i`, and the mapper's own placed field at that index carrying that member's name, which is the bridge from the description's `order` to the `t[i]` `write_struct` actually uses. A concept whose shorthands are all ARRAYS would compare nothing element by element, so that fails too rather than quietly checking less; none of the ten is that shape at any committed pin. Red-proven three ways, each injecting the defect it names rather than perturbing the test's own counter: a third element added to `Vector`'s tuple, `BoundingBox`'s `order` swapped so element 1 indexes a `RealOrientation`, and `mapFields` made to drop a member — the last of which reddens the count on seven concepts and, on `Vector3D`, reports `placed "z" at index 1 where `order` says "y"`, which is the silent misalignment stated exactly.

**A TIER-2 VALUE IS NOT NORMALISED AND CANNOT BE, which is the one place the flag does not reach.** `write_dyn` classifies a Lua table by SHAPE — a positive `#v` equal to the total pair count is `DYN_ARR`, anything else is a map — so one of these concepts arriving through a `Value` (a union the bindings cannot give a fixed layout, a `remote.call` result, an `Any`-typed read) still crosses in whichever form the engine sent, and there is no descriptor in that path to carry a `pos` flag. `docs/factorio-api.md`'s "the bindings do not ask you to know" is scoped to the TYPED path, and now says so.

**Three paths were checked rather than assumed, and none needed a change of its own.** Event payload encoding (`fk_mod.lua`'s `H.write_struct(fields, buf, e)`) and the configuration-changed payload are named-field blocks at the top level and reach a nested positional struct through the same `write_value`. `fk.bulk_get` is not affected at all: `BULKST` holds the scalars and the handle, and `K_STRUCT` is absent from it, so a struct-returning attribute is not bulk-eligible in the first place. **`read_struct` is untouched** — the guest-to-host direction keeps producing the keyed form, which the engine accepts for every one of these concepts.

**`positional_struct_positions` is a census row**: how many struct positions in the packaged member and event tables carry the flag, counted over every block `LuaSourceWith` renders and recursing through nested structs, array elements and dictionary keys and values. **568 at 2.0.77, 831 at 2.1.12, 834 at 2.1.14, 2.1.16 and 2.1.17.** Positions rather than concepts, deliberately: the concept count is ten and dull, and what a mapper change could quietly lose is the flag on a nested element. **It counts positions that CARRY the flag, which is not the same as positions where it is READ, and the two differ by about half**: an `args=` block only ever reaches the host through `read_struct`, which produces the keyed form and ignores `pos` by construction, so at 2.0.77 the 568 is 278 argument positions plus 1 typed-argument position that nothing consults against 225 returns and 64 event-payload positions that `write_value` does, and at 2.1.17 the 834 is 282 + 1 against 482 + 69. About 2.5 KB of inert `,pos=true` ships in argument descriptors, and it ships on purpose: the descriptor is one statement about the SHAPE of a value rather than about a direction, so a direction-aware rule would mean two renderings of one block and a side to get wrong, and the day a host path starts writing from an argument descriptor the silent answer would be zeros again. It is asserted **non-zero at every committed description**, because a row that read 0 would look exactly like a description that stopped carrying the shape — this file's own standing lesson, met again.

**The ABI SIGNATURE MOVES, and it should**: the descriptor text is what `APISignature` digests, so `pos=true` is part of the wire by the same argument the typed block is. **What the digest covers is the RENDERED descriptor**, `Placed.LuaTable()` verbatim, and this is the first change to exercise that distinction. Measurable rather than asserted: delete `placedList`'s `if f.Positional` block, all three lines of it, and `APISignature` reproduces the PREVIOUS digest exactly — `a130c3ec9c74` at the default pin, and its own value at each of the other four — so no id, offset or size moved and the signature moved anyway. Correct, because the flag is how the HOST reads the wire and a guest packaged against a table without it silently gets the old behaviour back. The consequence is that the warning every field guest now sees is WIDER THAN THE TRUTH for this change: it says a guest can call *different members*, which is the id case, where here the ids are right and the reading of a block is not. It stays generic, because wording it per case would be the digest classifying its own diff, which is the one thing a whole-table digest cannot do; the repair it prescribes, regenerate and rebuild, is the repair for every case alike. At the default pin `fk_api_sig_a130c3ec9c74` becomes `fk_api_sig_03170c04bf2e`, in both languages. **Nothing else in either binding set changed** — the flag is host-side, the guest struct is the same bytes at the same offsets — so the golden diff is one export name per language plus one census row per description. A guest built before this is warned about and still packages, which is what that digest's warning-not-refusal rule is for.

*Enforced by `TestAPositionalStructAcceptsEitherForm`, which drives the real `fk_abi.lua` under `lua52f` over seven legs: the array form, the keyed form, a table carrying both (the name wins), a **negative control** with no `pos=` given the array form that must still read zeros, and BoundingBox's nested shape in the array, keyed and mixed forms. By `TestTheTableOrTupleConceptsRenderPositional`, which derives the population from every committed description independently of `canonicalUnion`, fails on zero matches, and checks a table-only concept is not flagged. By `TestThePositionalCensusRowIsNonZeroEverywhere`. And end to end through the real generated decoders in both languages by `TestArraysCrossInBothDirections`, whose stub returns `inserter_drop_position` as `{2.19, 0}` beside the keyed `autopilot_destinations` it already returned — the two forms of one shape in one run. Red-proven: with the fix reverted, the three legs that carry the array form redden and read zeros — `array`, `box arr` and `box mix` — while `keyed`, `both`, `box key` and the negative control stay exactly where they were. Four of the seven green is the RESULT, not a weak proof: the keyed legs must not move, and a fallback that reddened them would be a general one rather than a flag the description earns.*
...
`LuaGameScript::create_surface(name, settings?)` was deferred whole because its optional `MapGenSettings` carries dictionaries — so a mod could not create a surface at all, and a genre (hidden-surface mods, Factorissimo-alikes) sat behind an argument almost nobody passes. It bound as `CreateSurface(name string) (Object, error)`, with the optional argument omitted and the reason in its doc comment, and it takes the whole thing today: `CreateSurface(name string, settings *MapGenSettings)` / `create_surface(&self, name: &str, settings: Option<MapGenSettings>)`. **The rule below is what mattered and it outlived the case that motivated it.**
...
**`MapGenSettings` is reachable and this paragraph used to say it was not.** It read *"`MapGenSettings` remains unreachable, and the way to reach it is dictionary fields inside a struct — the same capability the 5 deferred event payloads want"*, and that capability landed in the very round below ("A dictionary field inside a struct"), which took the event payloads from 5 deferred to 0 and `MapGenSettings` with them. The union-keyed `autoplace_settings` needed the tier-2 KEY case, which landed beside it. So `CreateSurface` takes the full argument today; the sentence outlived its own fix by several milestones and was found by BetterBeltBalancer's recon, which compiled a guest calling `CreateSurface` with the whole 13-key literal to check. **The optional-argument omission above is still the general rule and is what a caller sees when a shape really cannot be expressed** — it is simply no longer this shape.
...
### ...and the index operator's WRITE half — `obj[k] = v` — **built, both toolchains**

**This section used to end "there is no write half, and that is the description's doing".** That is true of the SCHEMA and was never true of Factorio: an operator carries a `read_type` and never a `write_type`, and the description says what a write DOES in prose instead. `LuaFluidBox::index`'s own description is *"Access, set or clear a fluid box… Writing `nil` removes all fluid from the fluid box"*, and `LuaSettings::global`, `::player_default`, `LuaPlayer::mod_settings` and `LuaSettings::get_player_settings` each carry *"individual settings can be changed by overwriting their ModSetting table"* — the last with the assignment written out as an example. So the gap was a generator reading the schema and not the sentence beside it, which is "the shapes the description carries and no generator read" one level further in.

Filed by BetterBeltBalancer (item 23), which needs `settings.global["name"] = {value = true}` — **the only way a mod changes its own runtime-global setting** — to turn its own setting on when it adopts a save written before that setting existed. Measured downstream on Factorio 2.1.14 before any of this was written: the write takes effect from any script context but an `on_init` dispatch, persists through a save, stays per-save rather than reaching `mod-settings.dat`, raises `on_runtime_mod_setting_changed` **synchronously** with no `player_index` in the payload, and answers *"LuaCustomTable doesn't contain key …"* for a key that is not a defined setting. `settings.startup` answers *"LuaCustomTable is read only"*.

**`M.IDXSET = 8`, and it is a kind for IDX's own sentence read one direction over.** `MemberSet`'s value is its only argument and its NAME comes out of the generation-time member table, so it has nowhere to put a key; `MemberIndex` has nowhere to put a value. The branch sits with the other three operators, before the member read, and is four lines: bind `local k, v = ...`, `pcall(index_set, obj, k, v)`, a status on a raise, `M.OK`. **Nothing comes back** — an assignment is not an expression in Lua and this ABI does not invent one, so a caller who wants to know what is there now asks `Get`, which is a second host call it did not have to make.

**An ABSENT value is a real nil and is the point.** `M.call` already trims to the last argument PRESENT, so a member whose value the description declares optional — LuaFluidBox's, whose prose spends a sentence on what nil MEANS — arrives with `v` nil and clears the slot. No special case anywhere; the general rule already said it.

| class | bound as | key | value |
|---|---|---|---|
| `LuaCustomTable` | `Set(Value, Value)` / `set(&Value, &Value)` | tier 2 | tier 2 |
| `LuaFluidBox` | `Set(uint32, *Fluid)` / `set(u32, Option<Fluid>)` | `uint32` | optional `Fluid` |

Both derive from the READ operator through one function, `indexKey`, and the value is the reader's own return type with its own optionality. The description gives an operator exactly one type; a write half that took a different one would be the generator inventing a signature, and a key derived twice would be two answers to one question.

**THE ALLOWLIST IS THE DESIGN DECISION, and it is `indexWriteHalf` in `gen.go`** — one row per class that declares an index operator, `true` or `false`, with the sentence it was read from. The alternative, emitting a setter for every index operator and letting the engine refuse the read-only ones, is *correct at runtime*: a refusal is `ERR_CALL_FAILED` carrying the engine's own words, exactly like every other raise here. It was refused anyway, on this file's own rule — **a skipped member is skipped, never faked**. `inventory.Set(1, stack)` would be a binding that exists, always fails, and reads to anyone grepping the generated file as a thing the API has; there is no such assignment in Factorio, and the way into a slot is `inv[1].set_stack(…)`.

**A `false` row is as load-bearing as a `true` one**, and `TestEveryIndexOperatorHasAWriteVerdict` is why: "no entry" must not be allowed to mean "not writable", because that is indistinguishable from "nobody looked" — which is precisely how eleven class operators stayed invisible for five milestones. A pin that adds an index operator fails there, with a message saying to go and read the prose. `LuaGuiElement` ("Gets children by name"), `LuaInventory` and `LuaTransportLine` are the three `false` rows and each carries its reason.

**Writability is per RECEIVER and not per class, which is why the doc comment carries the list.** `settings.global` accepts a write and `settings.startup` does not; both are the same `LuaCustomTable` and therefore the same member id, so there is no gating a generator could do that makes the emitted member always-correct. That also settles the asymmetry: being wrong in the `true` direction costs a member that answers a status, which some receivers answer anyway; being wrong in the `false` direction costs a gesture a guest cannot make at all. A row moves on evidence, not on symmetry.

*Enforced by `TestEveryIndexOperatorHasAWriteVerdict` (the enumeration), `TestTheWritableIndexOperatorsGetASetter` (the member table and both backends, with the key and the value — optionality included — checked against the READ operator's), `TestTheIndexSetKindMatchesTheABI`, `TestAnIndexAssignDispatches` (four legs through the real `fk_abi.lua` under `lua52f`: a tier-2 key with a tier-2 TABLE value, the read operator seeing what the write left, a raising `__newindex` coming back as a status with the engine's text in `last_error`, and an absent value reaching the metamethod as nil) and `TestAnIndexSetterPrunesAndReachesTheEngine`, which drives the downstream path end to end — `LuaSettings::global` as a handle, then the write — through a real packaged `control.lua` against an engine-shaped `settings`, shipping **811 bytes of member table for 2 members of 4,259**. Confirmed to fail with the `M.IDXSET` branch removed (`ERR_NO_MEMBER` on every leg and the guest writes nothing), with a verdict row deleted, and with a `false` row flipped to `true`.*

**And verified in a real Factorio, both languages, byte-identical log lines** (`scripts/run-guest.sh` with `GUEST=api`, and `LANG_=rust`): `index-assign: settings.global[undefined] refused true`, deterministic across two runs, with the leg after it still logging — which is the property the in-game arm is for. `examples/api` declares no settings of its own, so what a headless run can reach is the REFUSAL: the engine's *"LuaCustomTable doesn't contain key"* coming back as a status rather than unwinding through the wasm frame. A successful write needs a mod setting, which needs a data stage, which `run-guest.sh` does not package; the downstream measurement above is where the success side is measured.
...
#### ...and the doc comment says WHICH KEYS the tail takes

**The typed form said the variant tail goes in `extra` and named nothing that goes in it, which is a parameter name and not an answer.** `LuaSurface::create_segmented_unit` declares four ordinary parameters and two variant groups (`position-and-direction`, with `position` required, and `body-nodes`, with `body_nodes` required); the generated `LuaSurfaceCreateSegmentedUnitArgs` carries the four, so a reader had no way to learn that `position` is a parameter of the method at all. **Reported by WormholeBelts as item 11 of its gaps ledger, whose first draft of that entry concluded the typed path was unusable** -- which is the honest reading of what the crate said.

**ONE HELPER, FOUR RENDERINGS, which is this repo's own "one function, three callers" applied to a fact rather than to a number.** `variantdoc.go` reduces a method's groups once -- group name, its parameters in the description's `order`, the required ones marked, the group's own first sentence when it has one -- and that listing is rendered into the TYPED variant's doc comment, the generated Args struct's doc comment and the PLAIN tier-2 form's doc comment in both backends, plus `fklua docs`' group table, which now consumes it instead of walking the description itself. Three walks would be three chances to disagree about what a group contains.

**All three sites, because each is a different reader's entry point.** The typed form is where somebody reaches for the block and finds a parameter missing from it; the struct is where they read the fields and would otherwise take four fields for the whole parameter list; the plain form is where the groups are keys of the ONLY argument, so without the listing the member's entire tail is unnamed.

**FOUR SPACES OF INDENT ON EACH ENTRY, AND IT IS A RENDERING DECISION.** Both doc systems read an indented block as preformatted -- gofmt rewrites it to a tab and godoc shows a code block, four spaces after Rust's `/// ` is a CommonMark indented code block -- and two spaces would be a lazy paragraph continuation in rustdoc, folding the whole listing into the sentence above it with its line breaks gone. A blank comment line goes first, because an indented block cannot interrupt a paragraph -- emitted by the RUST backend, and supplied for the Go one by `go/format`, which inserts the separator and rewrites the four spaces to a tab when `gogen` runs the whole file through it. The committed Go source carries the shape; the Go generator never wrote it. The listing carries no backticks of its own; the Go site replaces any the DESCRIPTION brought, which is `TestNoBacktickReachesTheGeneratedSources`' standing constraint met in the one place it applies.

**AND THE RUST SIDE FENCES IT AS ```` ```text ````, BECAUSE AN INDENTED BLOCK IN A `///` COMMENT IS A DOCTEST.** godoc reads the indent as preformatted and stops there; rustdoc reads it as preformatted AND collects it, compiles it as Rust and runs it -- so the twelve listings arrive at `cargo test --doc -p fkapi` as twelve programs saying things like `body-nodes: body_nodes (required)`, and every one fails to compile. **Measured on the committed crate: 16 doctests failing before the fence and 4 after**, the remaining four being the hand-written `subscribe_*` examples in the preamble, which are older than this and out of scope. The `text` info string is the documented way to say "not Rust": rustdoc does not test it, and the rendered block loses a Run button it should never have had. The indent stays INSIDE the fence, because it is what groups a wrapped continuation line under the entry it continues, so the two backends' listings still differ only in the fence. `rustListing` is the one caller that wraps `VariantGroupLines`, which is why the fence is a function rather than a flag; the fence is also the only backtick the shared file emits and it is Rust-only, the same split `goDocText` is.

**SOURCE ONLY.** No member id moves, no census row moves and `fk_api_sig_*` is `a130c3ec9c74` before and after in both languages: the listing is documentation, exactly as `Member.Doc` and `FieldSpec.LazyPayload` are, and neither `LuaSourceWith` nor `APISignature` reads it. The golden diff is 1,013 added lines, every one of them a comment, and no line deleted or moved.
...
**AND THAT RESIDUAL IS NOW MEASURED AND REFUSED, on this very block.** A batch of these blocks plus a per-batch string pool, prototyped in `scratchpad/r2/harness.lua` and timed over a whole fifty-element window against `M.call_typed` on the same corpus, is **0.655× — six runs, 0.639-0.680 — against a rule of ≤ 0.6× fixed before the measurement.** It is an UPPER bound on the win: the prototype has no parent column, writes no handles back and skips everything `M.call_typed` pays, and both corrections that can be applied move it toward 1.0 (0.604 crediting the batch with the entire guest half of a blockless host call, 0.668 adding the engine's own ~2 µs per element to both sides). **The pool is worth 1.335× rather than 4b's 12×**, by ablation: two of the five audited fields are unions and cannot pool, and of the three that can only `type` and `style` repeat — a GUI element's `name` is what a handler finds it by. So the block stays a block, `fk.batch_add` is not built, and this paragraph is the record of why rather than a note that nothing forecloses it.

### Reading ONE attribute off MANY handles — `fk.bulk_get`

**A guest that polls pays a whole host call per entity, and a host call is the unit this cost model is denominated in.** Measured end to end through a real compiled guest under `--persist=table`, a scalar attribute read is **1,943 ns above a dispatch that makes no host call**, so a thousand-entity poll is most of a tick. `fk.bulk_get(member, handlep, count, dstp, retp)` pays the crossing once.

**IT IS A NEW IMPORT, AND THE PRUNING SCAN IS WHY.** Every other extension here has come out "kind" on one argument — the id stays an ordinary `i32.const` at the call site, so `UsedMembers` keeps working — and the first half of that argument holds here while the second decides the shape. Three options:

| shape | what it costs |
|---|---|
| a member KIND, one bulk id per eligible attribute | pruning unchanged; **+1,533 members** at the GA pin, a third of the table again, in every save and every download |
| ONE bulk member, the target id inside the ARGUMENT BLOCK | no new members; **pruning breaks silently** — an `i32.const` stored to memory is not an operand of an import, so `usedIDs` cannot see it |
| **a NEW IMPORT** | **no new member ids; pruning works by construction** |

The third is right and it is not a compromise: `usedIDs` is already parameterised by import name and operand position, so a third call with `("bulk_get", 0)` is four lines and `api check`'s `GuestSurface` needs no change at all. **Measured with the third scan removed**: a bulk-only guest packages `-- Generated by fklua. This guest calls no API members.` and an EMPTY member table — which is worse than shipping everything, because the mod loads, runs, and answers `ERR_NO_MEMBER` on every read. With the scan, the same guest packages **678 bytes for 1 member of 4,273**.

**THE DESTINATION IS AN ARRAY OF THE GETTER'S OWN RETURN BLOCK**, `count` copies at `retsize` stride, and that is the decision that makes the feature small. Element *i* is byte for byte what a single `fk.call` would have written at `retp`, so optionality is the presence byte `Layout` already places, the guest's decoder is the one its single getter already carries, a handle return mints transient handles through the same `encode_rets` path, and **nothing new is laid out anywhere** — `layout.go` is untouched.

#### Which attributes, and why not the others

`BulkEligible` is asked once and both backends read it, because two backends deciding for themselves is the AD5 shape and here the disagreement's symptom is a green binding the RUNTIME answers `ERR_NO_MEMBER` to. The rule is entirely the member's own signature: a readable attribute (`GET` or the custom-table twin `GETH`), exactly one return value, of a kind `HostAllocatesFor` says is fixed-width. That predicate is reused rather than restated, and it **fails closed** — a wire kind added at a later pin is ineligible until somebody says otherwise, which is the direction this feature wants.

| | 2.0.77 | 2.1.17 |
|---|--:|--:|
| `bulk_read_members` | **1,533** | 1,797 |
| `bulk_variant_bindings`, each language | **1,533** | 1,797 |

**Strings and containers are refusals with a reason.** A string element is a `(ptr, len)` into the 4 KiB scratch region, and a thousand of them exhaust it and fall through to `fk_alloc` per element — the allocation the region exists to remove, so a bulk string read would be dearer per element than the per-call form. A container element would be a nested `(ptr, count)` into the arena, so the destination would stop being a flat array, which is the property everything above rests on. `NameIs` already answers the dominant string question host-side.

#### Two arms, and the fast one hoists the READ and not the WRITE

The general arm is one memio accessor, one `M.get`, one `check_valid`, one `pcall` and one store per element. The fast arm is taken when the handle array is a contiguous 4-aligned run wholly inside shard 0, and it hoists that run's bounds check, alignment test and shard select — which is the loop guard's own argument one level up, applied to the host side of one crossing.

**IT DOES NOT HOIST THE STORE, and that is a deviation from the design this implements.** The draft measured a prototype that wrote straight into the shard table and reached 0.044×. That write bypasses `st32`'s dirty-page mark, which is `--persist=packed`'s save set AND `--gc=collected`'s write barrier — the defect class CLAUDE.md already gates the inlined 8-byte store on, met from the host side. `MEMPACK.mark` lives inside the generated chunk and nothing in `fk_abi.lua` can reach it, so the store goes through the funnel and **the last factor of two is not taken**. Being wrong about a precondition costs the general arm and never a wrong answer, which is the same failure direction the guard has.

#### Errors are per element, and a skipped element is written as ZERO

A dead handle, an invalid object or a raise clears that element's presence byte if it has one, writes the field's zero, and the walk CONTINUES; the count comes back in the return block and `lastError` carries the last raise. Skipping rather than aborting is chosen on ergonomics — a poll over a thousand entities of which one died between the search and the read is the ORDINARY case — and it is deterministic either way: the walk is index order over an array the guest supplied and nothing iterates a hash table.

**Writing the zero is not decoration.** The destination is a buffer the guest reuses, so an element left untouched reads back as the PREVIOUS crossing's value — silently plausible, and the worst answer available. For a MANDATORY attribute a zero that was skipped and a zero that was read are still the same bytes, which is a real loss of resolution and is stated rather than papered over: the count is what says something failed, and the presence byte is what says WHICH for the 415 eligible attributes the description marks optional.

The status is about the CALL: `ERR_NO_MEMBER` for a member that is absent or is not a readable attribute of an eligible shape, `ERR_BAD_ARGS` for a span that does not fit in the guest's memory — checked ONCE up front rather than trapping halfway through a destination it has already half written.

#### The binding is a PACKAGE-LEVEL function over a `[]Object`

`LuaControlSurfaceIndexBulk(objs []Object, dst []uint32) (int, error)`, and `lua_control_surface_index_bulk(objs: &[Object], dst: &mut [u32])`. There is no receiver because the receivers are the array, and the element type is `Object` because **that is what every array-of-handles return in the API is** — `goScalar` renders `KindHandle` as `Object` and there is no per-class element type anywhere. The first cut took a `[]<Class>` and needed the inherited members re-rendered per subclass (+455 functions at the GA pin); the docs snippet, compiled by TinyGo rather than reviewed, is what said that `FindEntitiesFiltered` returns a `[]Object` and the whole flagship example therefore did not typecheck. Taking `[]Object` settled inheritance by removing the question.

An OPTIONAL attribute's element carries a presence byte, so its destination is one of at most eleven generated structs named after the Go or Rust type (`BulkOptFloat64`, `BulkOptU32`), registered on demand and **size-asserted at build time**. `u64` is carried as its two halves with a reader over them, because this ABI aligns a u64 to 4 and both languages may align it to 8 — which would move the field off the offset the host writes at, silently, and only on some targets.

**A bulk read allocates NOTHING.** The count comes back through one package-level `uint32` (a `static mut` in Rust) rather than an arena block, which is `fk.LastError`'s own reasoning: a local whose address is taken does not stay on TinyGo's stack, and a block would be an arena bracket per call for four bytes.

#### What it measures

Host side, through the real `M.bulk_get` under `bin/lua52f` (`scratchpad/r4/bench.py`, output in `scratchpad/r4/RESULTS-shipped.txt`), against the per-call form at 998 ns/element:

| N | per call | fast arm (u32) | ratio |
|--:|--:|--:|--:|
| 1 | 985 ns | **589** | 0.598× |
| 2 | 991 | **399** | 0.402× |
| 8 | 1,002 | **255** | 0.254× |
...
**There is no size at which a guest is worse off, so there is no threshold to document.** The general arm is 0.545× at N = 200 and the optional-f64 arm 0.423×.

End to end through a real compiled guest (`examples/callcost`, probes 10 and 11), as ns above a dispatch that makes no host call:

| | per call | bulk of 4 | bulk of 256 |
|---|--:|--:|--:|
| `--persist=table` | 1,943 ns | 593 ns/el (**0.305×**) | 465 ns/el (**0.239×**) |
| `--persist=packed` | 125,073 ns | 21,715 ns/el (0.174×) | **1,035 ns/el (0.0083×)** |

**The packed column is the direction the design predicted and did not measure.** Packed costs ~40 µs per page actually WRITTEN; a per-call return block scatters and a bulk destination of 256 `u32` results is one page. `agents/abi.md` says not to quote either packed figure as universal and this does not — it is recorded as a direction, and it is a direction that only helps.

#### Why the shipped fast arm is 0.22× where the prototype reached 0.044×

The prototype dropped two things the shipped one keeps, and both are contracts rather than overhead: the **per-element `pcall`** (~87 ns, and it is what makes "an element that raises costs its own element" true) and the **store funnel** (~64 ns, and it is the dirty-page set). 216.8 − 87 − 64 = 66 ns against the prototype's 41-45 ns hoisted arm plus the `valid` check, so the staircase closes.
...
### A struct that is a BOX around one tier-2 value gets typed readers

`ModSetting` is a described table concept with exactly one mandatory field, and that field's type is a union — so it crosses as a `Value` and the generated struct is a box around a tagged union. A guest reading its OWN runtime setting therefore switched on a tag to learn whether its own boolean was on, which is the thing bindings exist to spare it. `Bool`/`Num`/`Str`/`Obj` in Go and `as_bool`/`as_num`/`as_str`/`as_obj` in Rust delegate to the `Value` accessors above and inherit their contract exactly, absent and mismatched alike answering not-ok.

**A RULE OVER THE LAYOUT, not a list of names.** `IsDynValueStruct(StructBlock)` is *one mandatory field, of kind dyn*, and both generators ask that one function — so they cannot disagree about what matched, and the same shape arriving at a later pin under another name gets the readers rather than silently getting nothing. **MANDATORY is part of the shape**: an optional single field would make the readers answer for a value that may not be there at all, and there the presence byte (the guest's own pointer, or `Option`) is the honest way to ask.

**`dyn_value_structs` is ONE census row for BOTH languages**, unlike the six member/event/define rows beside it, and that is a claim rather than an economy: the predicate is shared, so a disagreement would be a defect and not a language difference. The number is taken from the Go side and the Rust side is compared against it by a test rather than written down twice. It is **1** at every committed description. The row exists because a predicate that matched nothing would emit nothing, move no golden and pass every other gate here — the standing rule about a zero nobody writes down.
...
The full table is **~890 KB for 4262 members** at the 2.0.77 pin. Putting that in every mod would make a guest that calls five members carry more than four thousand it never touches — in every save, in every download, and in Factorio's parse time at load. Measured: **914,006 bytes for the whole API and its event table, 860,332 for the member table alone, and 2,513 for five members.** The same three numbers at the 2.1.14 pin are **1,100,428 / 1,044,482 / 1,902** — the table scales with the description, so quote the pin with the number.

**The five-member figure said 1,288 until the fkipc closeout and the origin of that number is unknown**, which is worth one sentence rather than a quiet edit. It was not any committed pin measured the way `TestAPrunedTableIsTiny` measures it — the same five member ids rendered as 1,359 B at 2.0.77, 1,624 at 2.1.12 and 1,902 at 2.1.14, and the first of those was the default rather than a coincidence — and it is not a units or an events-table difference either. **And 1,359 is 2,513 at that same pin since the index-assign round, which is worth knowing before anyone reads the figure as a trend**: the test names five member IDS, ids are dense sorted indices, and two members inserted mid-table make the same five ids name five different members. It is a fact about which five, not about the table; read it as an order of magnitude against the 860 KB, which is what it is for. The two halves of the old sentence were also measured through different functions, which is the likelier shape of the mistake: the whole-API number is `LuaSourceWith` (members plus events) and the five-member one was `LuaSource`. Both numbers above now come from that one test's own log line, which is where a reader should take them from.
...
### `fk_on_configuration_changed` carries a PAYLOAD, and it is the only hook that does

**The hook told a guest that something about the mod set moved and never what.** `script.on_configuration_changed` hands its handler a `ConfigurationChangedData` -- `mod_changes` keyed by mod name with the old and new versions, `mod_startup_settings_changed`, `migration_applied`, `migrations`, and the map's own old and new version -- and the FkLua hook dispatched with no arguments. Four of the thirteen mods the temptations survey audited branch on `mod_changes` directly.

**Nothing in the API references the concept, which is why it had never generated and is the whole of the fix.** It is a described `table` concept, so `mapFields` and `LayoutStruct` produce a layout exactly as they do for an event payload and `H.write_struct` writes it into the same per-level scratch buffer. `GenerateEvents` asks for it BY NAME -- there is no member to reach it through -- and a description that stops carrying it leaves `EventReport.ConfChanged` nil and the hook dispatching with no argument, which is what it always did.

**In the EXISTING hook rather than a sibling export.** A wasm export of no parameters compiles to a Lua function of no parameters, and Lua discards extra arguments, so a guest already in the field is called with a pointer it never looks at and behaves exactly as it did. That is asserted through the real `control.lua` rather than argued from the language rule.

**Pruned by an EXPORT, and it is the one entry in the packaged table that no constant scan can decide.** Members, events and defines are pruned by finding an i32 constant reaching an import -- the guest ASKS, so the ask is in the wasm. Nothing asks for this: Factorio raises the hook and hands it over. So the key is whether the guest exports `fk_on_configuration_changed`, spelled once as `ConfChangedHook` and derived from `Hooks` rather than written beside it. **`event_scratch` moves only when the layout is packaged**, because the buffer is sized to the largest thing encoded into it and folding the payload in unconditionally would move that number for every mod in existence. At every committed pin it fits anyway: 44 bytes against the largest event's 128.

**`hook_payload_fields` is a census row of its own**, because the payload is in none of the others -- not a member, not an event -- and its failure mode is that the hook fires with no argument and nothing says so. 0 at every pin before, 6 at every pin now.

*Enforced by `TestTheConfigurationChangedPayloadReachesTheGuest` (a wat guest reading `mod_changes`' first key at the offsets the packaged layout names, driven through the real `control.lua`), `TestANoArgumentConfigurationChangedGuestStillWorks`, `TestTheHookPayloadLayoutIsPrunedByTheExport`, `TestTheHookPayloadIsPackagedOnlyForAGuestThatExportsTheHook` and `TestBothBackendsEmitTheHookPayload` -- the last one being the parity half, which no census row can serve here for the same reason the payload needed its own. In a real Factorio by `scripts/run-confchanged.sh`, which packages ONE wasm at TWO mod versions and loads a save written by the first with the second, and reads back `mod fk-confchanged old=0.1.0 new=0.2.0` plus 42 entries of the nested `migrations` dictionary. Red-proven twice: dispatching without the pointer fails a payload-reading guest on nil arithmetic inside its own decode, and removing the export pruning packages the layout for a guest that can never be handed one.*

### `fk.subscribe` carries a NAME, and a custom input is the only thing addressed by one

**`LuaEventType` is a union of four arms and this import reached exactly one of them.** A keybind mod declares a `custom-input` prototype at the data stage and subscribes to it by that prototype's own NAME -- `script.on_event("my-input", f)`. Measured on 2.0.77 with a bare Lua mod: **`defines.events.CustomInputEvent` is nil while the table holds 233 other keys**, `script.on_event` with a known custom-input name is accepted, with an unknown one it RAISES `Unknown event name: <name>`, `script.get_event_id` on one returns a real number (219), and `script.raise_event` on one is REFUSED outright -- *"fkci-primary (ID 219) (219) can't be raised through script."*

The trap was that the description carries `CustomInputEvent` as an ordinary event, so the generator emitted a complete binding: the id constant, the payload struct, the reader, three field masks. A guest that found the right constant compiled, passed the pruning scan, and at load was told **this Factorio has no such event** -- a falsehood, about the one mistake it made. Nine of the thirteen mods the temptations survey audited subscribe to a custom input by name; one of them has no other entry point at all.
...
**A NAME THIS GAME DOES NOT HAVE IS A STATUS, AND THE REGISTRATION IS ROLLED BACK.** The engine raises, and taking a whole mod down at load for a typo in a keybind is worse than running without the keybind and saying so -- the call the filter path already makes one branch over. The rollback is the part that is not obvious: `registered[which]` and `filters[which]` are set before `script.on_event` is called, so leaving them behind would make a later subscription to the same name take the "already registered" arm, append, and **return success** into a dispatcher Factorio never installed. The pcall is on the NAMED path only, through an explicit `protect` parameter, so a numeric registration is byte-identical to what it was: it cannot raise on an id that came out of `defines.events`, and if it ever did, failing loudly is the right answer where silently not registering is not.

**And the false sentence is replaced by one that names both causes and the remedy**, because `defines.events[ev.name]` coming back nil has two of them and the likelier was being misdiagnosed as the other.

*Enforced by `TestASubscriptionByNameRegistersAndDispatches` (which logs the payload's own `input_name` back out at the offset the CustomInputEvent descriptor put it at, so one line proves the registration, the dispatch and the encode), `TestANameThisGameDoesNotHaveIsRefusedAndRolledBack`, `TestTheUnnamedCustomInputSubscriptionIsDiagnosedTruthfully` and `TestAThreeArgumentSubscribeStillRegistersByDefine`. In a real Factorio by `scripts/run-custominput.sh`, whose fixture is a control guest plus a DATA guest defining the prototypes -- the only way the success path is reachable, since the engine takes a string only when a prototype of that name is loaded. Red-proven three times: ignoring the name reports `registered false`; removing the rollback logs the engine's refusal ONCE for two attempts; and restoring the old sentence fails by name.*
...
- **One table plus array shorthands → the table.** It is the layout a guest sees and a write accepts either, so carrying only the table costs a guest nothing. The HOST still has to know both forms exist, because the engine sends whichever it likes; see "A `table | tuple` concept arrives in whichever form the engine sends" above.
...
**AND THE COLLAPSE IS NOW SAID AT THE CALL SITE, which is what the cost being ACCEPTED actually obliges.** `LuaForceCreateSpacePlatformArgs.planet` is an `Object` where the description says `SpaceLocationID`, which is `LuaSpaceLocationPrototype | string`: the string arm is unreachable from a guest, and because every handle in the package is one type, ANY object satisfies it -- a `LuaPlanet` compiles and the engine refuses it at runtime. Reported by WormholeBelts as item 4 of its gaps ledger. A `FieldSpec` that came from shape B now remembers the union it collapsed from (the concept's NAME where the position declared one, and the arms in the description's own spelling), and both backends render one line into the doc comment of the struct field and of the method parameter: *planet is declared SpaceLocationID (LuaSpaceLocationPrototype | string). Only the handle arm has a fixed layout, so this position carries only the LuaSpaceLocationPrototype handle. Find one under the prototypes global.* One helper, four call sites (`collapseddoc.go`), for the same reason `variantdoc.go` has one.

**THE CLASS IS THE INNER ONE WHERE THE ARM IS ITSELF SHAPE B.** `canonicalUnion` recurses through `mapType`, so an option that is a shape-B concept arrives already collapsed and already knowing its class -- and writing the option's own spelling over that produced "it takes the `PlayerIdentification` handle" on `LuaGameScript::ban_player` and `unban_player`, at every committed pin and in both languages. `PlayerIdentification` is `uint32 | string | LuaPlayer`, not a class: the note named a thing no guest can hold a handle to, which is the exact confusion it exists to prevent. The inner answer wins, because a collapse of a collapse still resolves to one class and that class is the one the engine accepts.

**THE NOTE SAYS NOTHING ABOUT DIRECTION, and that is a correction rather than a trim.** The first draft ended "the union's scalar arms name one on the way in and a guest cannot send them", which is shape B's own argument and is false at two of the positions the note renders on. A struct field's note is rendered wherever the struct is, and `ItemIDAndQualityIDPair` and `RecipeIDAndQualityIDPair` appear only in RETURNS -- nothing is being sent there at all, and four more structs are used both ways. And on the five fields of a variant-group method's typed argument struct (`create_entity`'s quality, force and player; `create_segmented_unit`'s quality and force) a guest sends the string arm perfectly well, through the PLAIN form of the same member. What is left is the half that is true everywhere: this position carries only the handle.

**TWO CONDITIONAL CLAUSES, AND EACH IS DERIVED RATHER THAN SPELLED.** The `prototypes` hint comes from `LuaPrototypes`' own attribute list at the pin, not from `strings.HasSuffix(class, "Prototype")`: the name rule is wrong in BOTH directions -- `LuaBurnerPrototype`, `LuaFluidBoxPrototype`, `LuaHeatBufferPrototype` and the four energy-source prototypes are reached through an entity prototype and are 7 of the 45 `*Prototype` classes that are not under `prototypes`, **so the rule is right about 38**, while `LuaModData`, the two noise classes and the item-group classes -- `LuaGroup` up to 2.1.14, split into `LuaItemGroup` and `LuaItemSubGroup` from 2.1.16 -- are under it and are named nothing of the sort. **The derived set is 42 classes at 2.0.77, 2.1.12 and 2.1.14 and 43 from 2.1.16**, which is a size and not a hit count; 42 was written here and in two code comments as if it were the name rule's, against a list of seven exceptions to 45. It fires today on eight classes at the GA pin (Damage, Decorative, Particle, Quality, RecipeCategory, SpaceLocation, SurfaceProperty, TrivialSmoke, all `Lua*Prototype`) and nine from 2.1.12, which adds SpaceConnection -- what the derivation and the name rule happen to agree on among the classes that actually collapse -- so no committed description can tell the two apart and the test that distinguishes them builds one that can. The tier-2 clause fires where the member is generated in two forms, which is exactly a variant-group method's typed argument struct, and it is recorded where that is known: `add()` has the parent `FieldSpec` and its `Variants`, and `emit()` has a struct name and no member.

**THE CENSUS COUNTS THE SENDING POSITIONS**, `collapsed_union_arg_positions`: **257** at the GA pin, 274 at 2.1.12 and 276 at 2.1.14 and above. Args and TypedArgs, not Rets -- a READ is not a collapse anybody pays for, since the engine returns the object and the scalar arms are ways of naming one on the way IN, which is shape B's own argument. Deduplicated the way the generators EMIT: a positional argument is one position per member, and a field of a NAMED struct counts once however many members take that struct, so the row moves when a collapse does rather than when a member is added.

**TWO ROADS NOT TAKEN, and both are refused on their cost rather than on principle.** Making the field TIER 2, the way `mapWriteType` does for an attribute's write half, is a WIRE change at every takes-table member carrying a ForceID-shaped field: the block's layout moves, so every such member's contribution to `fk_api_sig_*` moves and every guest in the field has to be rebuilt -- to buy an arm the tier-2 form of the same member already accepts. Typing the handle PER CLASS, so the field is a `LuaSpaceLocationPrototype` rather than an `Object`, is a rendering change across the whole API: every class-typed field and every handle return grows a type, both backends need a conversion wherever a handle is minted, and the payoff is a compile error on the subset of positions where a union named exactly one class. Neither is foreclosed; both are larger than the gap.
...
**`dispatch_done` releases the transient handles**, flushes packed memory and syncs globals, when the OUTERMOST guest entry point returns -- `dispatch` counts the depth and calls `leave_outermost` only at 0, so a nested event releases nothing. The transient release is the half that matters: it is what makes the dominant leak shape cost nothing.
